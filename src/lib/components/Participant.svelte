...
<div class="participant">
  <span aria-label="{($quality * 100).toFixed(2)}%" class="material-symbols-outlined">
    signal_cellular_{Math.min(Math.ceil($quality * 4), 4)}_bar
  </span>
  <span class="cutoff-text">{name}</span>
  <FFTGraph fft={$fft} />
  <PopoutButton bind:popout>
    <button aria-label="More" class="borderless" slot="activator" let:open on:click|stopPropagation={open}><span class="material-symbols-outlined">more_vert</span></button>
